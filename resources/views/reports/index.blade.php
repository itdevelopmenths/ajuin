...
{{-- Date filter --}}
<div class="card" style="padding:1rem 1.25rem;margin-bottom:1.25rem;display:flex;flex-wrap:wrap;align-items:flex-end;gap:.75rem">
    <form method="GET" style="display:contents">
        <div style="display:flex;align-items:center;gap:.5rem;flex:1;min-width:120px">
            <label class="form-label" style="margin:0;white-space:nowrap;font-size:.8rem">Dari</label>
            <input type="date" name="from" value="{{ request('from') }}" class="form-input" style="padding:.5rem .75rem">
        </div>
        <div style="display:flex;align-items:center;gap:.5rem;flex:1;min-width:120px">
            <label class="form-label" style="margin:0;white-space:nowrap;font-size:.8rem">Sampai</label>
            <input type="date" name="to" value="{{ request('to') }}" class="form-input" style="padding:.5rem .75rem">
        </div>
...
<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(240px,1fr));gap:1rem">
    ...
    $statusColors = ['SCREENING'=>'#f59e0b','IN_PROGRESS'=>'#3b82f6','PEMBAYARAN'=>'#f97316','EKSEKUSI'=>'#8b5cf6','SELESAI'=>'#14b8a6','REJECTED'=>'#ef4444'];
    @endphp
        ...
        <div style="display:flex;flex-direction:column;gap:.75rem">
            ...
            <div>
                ...
                <div style="height:5px;background:#f1f5f9;border-radius:999px;overflow:hidden">
                    <div style="height:100%;width:{{ $maxStatus > 0 ? round($count/$maxStatus*100) : 0 }}%;background:{{ $statusColors[$key] ?? '#6366f1' }};border-radius:999px;transition:width .4s"></div>
                </div>
            </div>
            @endforeach
        </div>
    ...
    <div class="card" style="padding:1.5rem">
        ...
        <div style="display:flex;flex-direction:column;gap:.75rem">
            ...
            <div>
                ...
                <div style="height:5px;background:#f1f5f9;border-radius:999px;overflow:hidden">
                    <div style="height:100%;width:{{ $maxType > 0 ? round($count/$maxType*100) : 0 }}%;background:linear-gradient(90deg,#6366f1,#8b5cf6);border-radius:999px"></div>
                </div>
            </div>
            @endforeach
        </div>
